...
const httplib = {
  request: async (opts: any, cb: (data: any) => any, errcb: (data: any) => any, maxtries: number = 5, timeout: number = 0, sleepms: number = 100) => {
    const port = opts.port || 80;
    const protocol = "http://"
    const ret = await ky(protocol + opts.host + ":" + port + opts.path, {
      ...
      ...(opts.rejectUnauthorized ? { rejectUnauthorized: opts.rejectUnauthorized } : {}),
      ...(opts.ca ? { ca: opts.ca } : {}),
      hooks: {
        beforeRetry: [
          //@ts-ignore
